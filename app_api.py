...
def load_year(year: int, force: bool = False, progress_cb=None) -> pd.DataFrame | None:
    ...
    If `force=True`, re-download even if the pickle exists on disk.
    Esta versión usa SOLO la API — no hay fallback a CSV local.
    """
...
with st.sidebar:
    ...
    # --- Descargar año histórico bajo demanda ---
    with st.expander("Descargar otro año (histórico)", expanded=False):
        _yrs_dl = sorted([y for y in RESOURCES_BY_YEAR.keys() if y != LIVE_YEAR],
                         reverse=True)
        _mani_state = _read_manifest()
        _rows = []
        for _y in _yrs_dl:
            _cached = str(_y) in _mani_state and _year_cache_path(_y).exists()
            _rows.append(f"- **{_y}** · {'en caché' if _cached else 'pendiente'}")
        st.markdown("\n".join(_rows))

        _pick_yr = st.selectbox("Año a descargar", _yrs_dl, key="api_pick_yr")
        if st.button("Descargar este año desde la API", width="stretch", key="api_dl_btn"):
            _prog = st.progress(0.0, text=f"Descargando {_pick_yr}…")
            def _cb(done, total):
                if total:
                    _prog.progress(min(done / total, 1.0),
                                   text=f"{_pick_yr}: {done:,}/{total:,} filas")
                else:
                    _prog.progress(0.0, text=f"{_pick_yr}: {done:,} filas…")
            try:
                df_new = load_year(_pick_yr, force=True, progress_cb=_cb)
                if df_new is not None and not df_new.empty:
                    _prog.progress(1.0, text=f"{_pick_yr}: {len(df_new):,} filas listas.")
                    st.session_state["_cache_token"] = datetime.now().isoformat(timespec="seconds")
                    st.cache_data.clear()
                    st.session_state["_last_refresh"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    st.rerun()
                else:
                    st.error(f"No se pudo descargar {_pick_yr} desde la API.")
            except Exception as e:
                _prog.empty()
                st.error(f"Falló la descarga de {_pick_yr}: {e}")
    ...
    st.markdown(
        ...
        los anteriores se descargan bajo demanda desde la API.<br>
    ...
    if not companies:
        ...
        if not _has_cache:
            if _yr_sel == LIVE_YEAR:
                ...
                st.warning(
                    f"No hay datos en caché local para {_yr_sel}. "
                    f"En la barra lateral, expandí **Descargar otro año (histórico)** "
                    f"y bajalo desde la API."
                )
        else:
            st.error(
                f"El caché local de {_yr_sel} está pero no se pueden listar empresas. "
                f"Probá re-descargar ese año o contactá al dev."
            )
